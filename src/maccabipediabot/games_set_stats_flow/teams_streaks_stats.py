...
from common import _USER_DATE_GAMES_FILTER_KEY, transform_teams_with_maccabi_games_to_telegram_html_text
from games_set_stats_flow.common_menu import go_to_more_stats_or_finish_menu
from games_set_stats_flow.games_stats_conversation_handler_states import select_teams_streaks_stats
from games_set_stats_flow.menus_keyboards import create_teams_streaks_games_stats_keyboard
from games_set_stats_flow.menus_options import TeamStreaksStatsMenuOptions
...
def show_teams_streak_maccabi_unbeaten_action(update, context):
    ...
    query = update.callback_query
    query.edit_message_text(parse_mode=ParseMode.HTML, text=f"היריבות עם הרצף 'לא ניצחו את מכבי' הארוך ביותר: \n{top_teams_html_text}")
...
def show_teams_streaks_maccabi_win_action(update, context):
    ...
    query = update.callback_query
    query.edit_message_text(parse_mode=ParseMode.HTML, text=f"היריבות עם הרצף 'מכבי רק ניצחה' הארוך ביותר: \n{top_teams_html_text}")
